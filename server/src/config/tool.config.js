   ...
   try {
      for (const toolConfig of availableTools) {
         if (toolConfig.enabled) {
            try {
               // Instantiate the tool safely
               tools[toolConfig.id] = toolConfig.getTool();
            } catch (toolError) {
               console.error(
                  chalk.red(
                     `[ERROR] Failed to initialize tool: ${toolConfig.id}`,
                  ),
                  toolError.message,
               );
            }
      ...
      // ✅ FIX: Always return object (never undefined)
      return tools;
   } catch (error) {
      console.error(
      ...
      // ✅ FIX: return empty object instead of undefined
      return {};
   }
}
...
export function enableTools(toolIds) {
   if (!Array.isArray(toolIds)) {
      console.log(chalk.red("[ERROR] toolIds is not an array:"), toolIds);
      return;
   }
